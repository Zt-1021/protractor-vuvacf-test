...
describe('Protractor Demo App', function() {
  ...
  it('board should have boardlist',function(){
    //定位首页“板块”
    var board = element(by.xpath('(//li[@class="ant-menu-item ng-star-inserted"])[2]'));
    expect(board.getText()).toEqual('板块');
    board.click();//进入板块列表页
    var boardlist01 = element.all(by.xpath('//div[@class="menu"]//li'));
    expect(boardlist01.count()).toBe(5);
    expect(boardlist01.get(0).getText()).toEqual(" 园区 ");//定位板块首个类型“园区”
    browser.sleep(3000);
    var boardname01 = element(by.xpath('//div[@class="content1 ng-star-inserted"][1]//span[@class="span1"]')).getText();
    expect(boardname01).toEqual("上海移动互联网创新园");//定位园区板块列表的首个园区“上海移动互联网创新园”
    boardname01.getText().click();//进入“上海移动互联网创新园”的信息页
    browser.sleep(3000);
    expect(board.getText()).not.toContain("企业");//登录账户非此园区的管理员，没有企业，生态，统计三块内容
    var title01 = element(by.xpath('//div[@class="left-middle block-color"]/div[@class="left-middle-title"]/h1[@class="board"]'));
    expect(title01.getText()).toEqual(boardname01);//判断信息页的标题与点击进入的板块是否一致
  });
...
describe("manager", function() {
  ...
  //var boardlist02 = element.all(by.xpath('//*[@id="appLay"]//div/div[2]/div'))//获取多个板块列表：板块名，类型，企业数量，管理板块，进入板块
  //var boardName = element.all(by.xpath('//*[@id="logotext"]'));//获取多个板块名字
  //var boardlist02 = element(by.xpath('//*[@id="appLay"]//div/div[2]/div'))//获取单个板块列表：板块名，类型，企业数量，管理板块，进入板块
  //var boardName = element(by.xpath('//*[@id="logotext"]'));//获取单个板块名字
  
  it('goto manager',function(){
    ...
    img01.click();
    browser.sleep(2000);
    expect(manager.getText()).toEqual("管理");
    manager.click();
    browser.sleep(3000);
    expect(element(by.xpath('//div[@class="left-title-wrapper"]/span')).getText()).toEqual("请选择要管理的板块");
  });//点击管理，进入后台，根据进入的页面是否有“请选择要管理的板块”进行判断
  ...
  it("edit the notify",function(){
    ...
    addNotify = element(by.buttonText("添加公告"));
    addNotify.click();
